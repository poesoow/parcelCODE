<template>
  ...
  <div v-if="trackingState === true && errorMsg === ''" class="w-full relative">
    ...
    <div v-if=" errorMsg === '' && Trackings.level !== 1">
      <div class="my-5 flex justify-around">
        <div class="relative" v-for="(level) in PostListName.length - 1 " :key="level">
          ...
          <p>{{ PostListName[level] }}</p>
        </div>
      </div>
      <div class="py-5">
        <div class="border-t-[1px] font-bold text-lg pl-5 flex items-center" :class="[themecolor[theme] && themecolor[theme].border]">
    ...
    <div v-else>
      <div class="font-bold text-lg pl-5 flex justify-center items-center">운송장 미등록 상태이거나 업체에서 상품을 준비중입니다.</div>
    </div>
  </div>
  ...
  data() {
    return {
      ...
      themecolor: {
        ...
        "green": {
          "back": "odd:bg-green-300 even:bg-green-100", "hover": "hover:bg-green-500", "active": "bg-green-400", "text": "text-white", "border": "border-green-400"
        },
        "pink": {
          "back": "odd:bg-pink-300 even:bg-pink-100", "hover": "hover:bg-pink-500", "active": "bg-pink-400", "text": "text-white", "border": "border-pink-400"
        }
      }
    }
  },
